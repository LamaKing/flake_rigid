...
Usage
-----
    flake map         -i params.yaml --grid  grid.yaml   -o output.h5
    flake sweep       -i params.yaml --spec  spec.yaml   --outdir sweep_out/
    flake string      -i params.yaml --cfg   string.yaml -o path.h5
    flake make-sweep  [options]  -o spec.yaml
...
def _cmd_map(args):
    ...
    print("flake map: type=%s  grid=(%d x %d)  n_jobs=%d"
    ...
    print("flake map: saved to %s" % out, flush=True)
...
def _cmd_sweep(args):
    ...
    spec.yaml keys:
        ...
        post_fn:        str  -- name from drift_velocity/drift_omega/mean_velocity/last_state
    ...
    # Strip CLI-only keys and base_md_kwargs (handled separately below);
    # pass everything else straight to sweep_md.
    ...
    print("flake sweep: done. %d runs written to %s" % (len(results), outdir),
          flush=True)
...
def _cmd_string(args):
    ...
    save_map(result, out, params=params)
    print("flake string: saved to %s" % out, flush=True)
...
def _cmd_make_sweep(args):
    ...
    print("flake make-sweep: wrote %s" % out, flush=True)
...
def _cmd_make_params(args):
    ...
    cmd_comment = (
        "# generated by: flake make-params --substrate %s --spacing %.6g"
        % (substrate, spacing)
    )
    ...
    if out == '-':
        ...
        with open(out, 'w') as fh:
            fh.write(content)
        print("flake make-params: wrote %s" % out, flush=True)
