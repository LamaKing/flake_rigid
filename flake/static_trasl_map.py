...
from flake.substrate import substrate_from_params, calc_matrices_bvect

def static_traslmap(pos, inputs, calc_en_f, name=None, log_propagate=True, debug=False):
    ...
    for i1, dda1 in enumerate(np.linspace(da11, da12, nbin, endpoint=True)):
        for i2, dda2 in enumerate(np.linspace(da21, da22, nbin, endpoint=True)):
            ...
            if it % printerr_skip == 0:
                c_log.info("it=%10.4g of %.4g (%2i%%) x=%9.3g y=%9.3g en=%10.6g" % (it, Nsteps, 100.*it/Nsteps, pos_cm[0], pos_cm[1], e_pot))
            # Assign step results
            data[it] = pos_cm[0], pos_cm[1], e_pot, forces[0], forces[1], torque
            # Print step results
...
if __name__ == "__main__":
    ...
    for Nl in Nl_range:
        ...
        tho = 0
        if 'tho' in inputs.keys(): tho = inputs['tho']
        pos = rotate(pos, tho) # Rotate at starting angle
        pos_vector.append(pos.copy()) # Just to be sure, pass a copy
        c_log.info("%s cluster size %i (Nl %i) rotated tho=%.4g" % (inputs['cluster_shape'], pos.shape[0], Nl, tho))
