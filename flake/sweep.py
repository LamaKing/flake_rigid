...
def drift_omega():
    ...
    Defined as (theta_final - theta_initial) / (t_final - t_initial),
    in degrees per time unit.  Zero when the cluster is rotationally pinned.

    Returns:
        callable post_fn(traj_dict, run_params) -> float.

    Example:
        results = sweep_md(..., post_fn=drift_omega())
        omega_drift = np.array([r['result'] for r in results])
    """
    def _post_fn(traj_dict, run_params):
...
def sweep_md(pos, calc_en_f, sweep_spec,
    ...
    def _run_one(i, kwargs):
        ...
        if run_dir is not None:
            traj_path    = os.path.join(run_dir, 'traj.h5')
            already_done = save_traj and os.path.isfile(traj_path) and not overwrite
            if already_done:
                warnings.warn(
